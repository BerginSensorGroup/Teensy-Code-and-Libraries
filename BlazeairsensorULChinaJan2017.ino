...
void getAlpha() {
  workNO = analogRead(workNOpin) * 3.22;
  auxNO = analogRead(auxNOpin) * 3.22;
  //  workCO = analogRead(workCOpin) * 4.9;
  //  auxCO = analogRead(auxCOpin) * 4.9;

  workNO2 = analogRead(workNO2pin) * 3.22;
  auxNO2 = analogRead(auxNO2pin) * 3.22;
  workO3 = analogRead(workO3pin) * 3.22;
  auxO3 = analogRead(auxO3pin) * 3.22;

  NO = ((workNO - zeroNO) - (1.37 * (auxNO - aeNO))) / sensitivityNO;
  NO2 = ((workNO2 - zeroNO2) - (1.8 * (auxNO2 - aeNO2))) / sensitivityNO2;
  //NO2 = ((workNO2 - zeroNO2) - (1.18*(auxNO2 - aeNO2))) / sensitivityNO2;

  O3 = (((workO3 - zeroO3) - (2.0 * (auxO3 - aeO3))) - (NO2 * auxNO2O3)) / sensO3;
  //O3 = (((workO3 - zeroO3) - (0.18*(auxO3 - aeO3))) - (NO2 * auxNO2O3)) / sensO3;
  ...
  worksumNO2 += workNO2 / 3.22;   //raw analog data
  auxsumNO2 += auxNO2 / 3.22;
  worksumO3 += workO3 / 3.22;
  auxsumO3 += auxO3 / 3.22;
